...
const speakerInfo: Record<Speaker, { label: string; color: string }> = {
  judge: { label: 'Judge Harmon', color: 'text-primary' },
  prosecutor: { label: 'Prosecutor Blake', color: 'text-destructive' },
  defender: { label: 'You (Defender)', color: 'text-gold-light' },
...
const CourtroomMain = ({ caseData, onVerdict }: CourtroomMainProps) => {
  ...
  return (
    <div className={`h-screen flex bg-background relative overflow-hidden ${shaking ? 'screen-shake' : ''}`}>
      {/* Red flash overlay */}
      ...
      <div className="w-80 flex-shrink-0 border-r border-border bg-card/90 backdrop-blur-sm flex flex-col z-10">
        <div className="px-4 py-3 border-b border-border">
          <h2 className="text-sm font-display text-primary">📜 Court Transcript</h2>
          <div className="flex items-center gap-2 mt-2">
            <span className="text-[10px] font-display text-muted-foreground">🎯 SCORE</span>
        ...
        <div ref={chatRef} className="flex-1 overflow-y-auto p-3 space-y-2">
          {dialogues.map((d, i) => {
            ...
            return (
              <motion.div
                ...
                className="text-xs"
              >
                <span className={`font-display ${info.color} font-semibold`}>{info.label}:</span>
                <p className="text-foreground/80 font-body mt-0.5 leading-relaxed">{d.text}</p>
              </motion.div>
            );
          })}
          ...
          {isTyping && currentText && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs">
              <span className={`font-display ${speakerInfo[currentSpeaker].color} font-semibold`}>
                {speakerInfo[currentSpeaker].label}:
              </span>
              <p className="text-foreground/80 font-body mt-0.5 cursor-blink">{currentText}</p>
            </motion.div>
          )}
        </div>
      ...
      <div className="flex-1 relative flex flex-col items-center justify-end"
        style={{ backgroundImage: `url(${courtroomBg})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
      >
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 flex flex-col items-center z-10">
          ...
          <CharacterSprite
            ...
            className="h-44"
          />
          <span className="text-xs font-display text-primary mt-1 bg-background/70 px-2 py-0.5 rounded">Judge Harmon</span>
        </div>
        ...
        <div className="absolute bottom-16 left-12 flex flex-col items-center z-10">
          ...
          <CharacterSprite
            ...
            className="h-52"
          />
          <span className="text-xs font-display text-gold-light mt-1 bg-background/70 px-2 py-0.5 rounded">You (Defender)</span>
        </div>
        ...
        <div className="absolute bottom-16 right-12 flex flex-col items-center z-10">
          ...
          <CharacterSprite
            ...
            className="h-52"
          />
          <span className="text-xs font-display text-destructive mt-1 bg-background/70 px-2 py-0.5 rounded">Prosecutor Blake</span>
        </div>
